...
CRONOTRIGGER_DRAFT_DIR="$CRONOTRIGGER_RUNTIME_DIR/drafts"
...
cronotrigger_init_paths() {
    ...
    mkdir -p "$CRONOTRIGGER_CONFIG_DIR" "$CRONOTRIGGER_LOG_DIR" "$CRONOTRIGGER_RUNTIME_DIR" "$CRONOTRIGGER_DRAFT_DIR" || {
        echo "cronotrigger: failed to initialize directories" >&2
        return 1
    }

    chmod 700 "$CRONOTRIGGER_CONFIG_DIR" "$CRONOTRIGGER_STATE_DIR" \
        "$CRONOTRIGGER_LOG_DIR" "$CRONOTRIGGER_RUNTIME_DIR" "$CRONOTRIGGER_DRAFT_DIR" 2>/dev/null || true
